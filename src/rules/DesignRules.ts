...
function detectHardcodedColors(code: string, file: string, primaryColor?: string): Issue[] {
  const issues: Issue[] = [];
...
function detectGenericGradients(code: string, file: string): Issue[] {
  const issues: Issue[] = [];
...
function detectPlaceholderOnly(code: string, file: string): Issue[] {
  const issues: Issue[] = [];
...
function detectEmojiIcons(code: string, file: string): Issue[] {
  const issues: Issue[] = [];
...
export const designRules = (tokens?: DesignTokenSet) => [createDesignValidatorRule(tokens)];
