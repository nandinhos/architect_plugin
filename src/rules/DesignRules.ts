...
function isRelevantLanguage(language: RuleContext['language']): boolean {
  return (
    language === 'html' ||
    language === 'css' ||
    language === 'typescript' ||
...
export function createHardcodedColorRule(tokens?: DesignTokenSet): BehaviorRule {
...
export function createGenericGradientRule(): BehaviorRule {
  return {
    id: 'DES-002',
...
export function createPlaceholderLabelRule(): BehaviorRule {
  return {
    id: 'DES-003',
...
export function createEmojiIconRule(): BehaviorRule {
  return {
    id: 'DES-004',
    ...
    validate(context: RuleContext): RuleResult {
      ...
      if (emojiRegex.test(code)) {
        issues.push({
          code: 'DES-004',
          message:
            'Emoji detectado em codigo. Use bibliotecas de icones profissionais (Lucide, Heroicons).',
          severity: 'low',
          file: filePath,
        });
      }
...
export function createDesignValidatorRule(tokens?: DesignTokenSet): BehaviorRule {
  return createHardcodedColorRule(tokens);
}

export const designRules = (tokens?: DesignTokenSet) => [
